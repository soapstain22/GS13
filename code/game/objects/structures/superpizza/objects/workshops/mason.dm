...
/datum/techweb/specialized/autounlocking/masonry
	design_autounlock_buildtypes = SMITHING
	allowed_buildtypes = SMITHING

/obj/item/circuitboard/machine/masonry
	name = "Masonry (Machine Board)"
	icon_state = "generic"
	build_path = /obj/machinery/masonry
	req_components = list(
		/obj/item/stock_parts/matter_bin = 3,
		/obj/item/stock_parts/manipulator = 1,)
/obj/machinery/masonry
	icon = 'code/game/objects/structures/superpizza/objects/workshops/workshops.dmi'
	name = "Masonry"
	desc = "It produces items using ROKS"
	icon_state = "masonry"
	density = TRUE
	use_power = IDLE_POWER_USE
	idle_power_usage = 10
	active_power_usage = 100
	circuit = /obj/item/circuitboard/machine/masonry
...
/obj/machinery/masonry/Initialize()
	AddComponent(/datum/component/material_container, SSmaterials.materialtypes_by_category[MAT_CATEGORY_STONE], 0, TRUE, null, null, CALLBACK(src, .proc/AfterMaterialInsert))
	. = ..()

	stored_research = new /datum/techweb/specialized/autounlocking/masonry
	matching_designs = list()

/obj/machinery/masonry/Destroy()
	return ..()

/obj/machinery/masonry/ui_interact(mob/user)
	. = ..()
	if(!is_operational())
		return
	...
	var/datum/browser/popup = new(user, "masonry", name, 400, 500)
	popup.set_content(dat)
	popup.open()

/obj/machinery/masonry/on_deconstruction()
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	materials.retrieve_all()

/obj/machinery/masonry/attackby(obj/item/O, mob/user, params)
	if (busy)
		to_chat(user, "<span class=\"alert\">The masonry is busy</span>")
		return TRUE

	if(default_deconstruction_screwdriver(user, "masonry_t", "masonry", O))
		updateUsrDialog()
		return TRUE
...
/obj/machinery/masonry/proc/AfterMaterialInsert(item_inserted, id_inserted, amount_inserted)
	if(istype(item_inserted, /obj/item/stack/ore/bluespace_crystal))
		...
		flick("masonry_r",src)//plays glass insertion animation by default otherwise
	else
		flick("masonry_o",src)//plays metal insertion animation


		use_power(min(1000, amount_inserted / 100))
	updateUsrDialog()

/obj/machinery/masonry/Topic(href, href_list)
	if(..())
		return
	if (!busy)
		if(href_list["menu"])
		...
		to_chat(usr, "<span class=\"alert\">The masonry is busy. Please wait for completion of previous operation.</span>")

	updateUsrDialog()

	return

/obj/machinery/masonry/proc/make_item(power, list/materials_used, list/picked_materials, multiplier, coeff, is_stack, mob/user)
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	...
	if(is_stack)
		...
		N.update_icon()
		N.masonry_crafted(src)
	else
		for(var/i=1, i<=multiplier, i++)
			var/obj/item/new_item = new being_built.build_path(A)
			new_item.masonry_crafted(src)
...
/obj/machinery/masonry/RefreshParts()
	var/T = 0
...
/obj/machinery/masonry/examine(mob/user)
	. += ..()
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
...
/obj/machinery/masonry/proc/main_win(mob/user)
...
/obj/machinery/masonry/proc/category_win(mob/user,selected_category)
...
/obj/machinery/masonry/proc/search_win(mob/user)
	var/dat = "<A href='?src=[REF(src)];menu=[AUTOLATHE_MAIN_MENU]'>Return to main menu</A>"
...
/obj/machinery/masonry/proc/materials_printout()
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
...
/obj/machinery/masonry/proc/can_build(datum/design/D, amount = 1)
...
/obj/machinery/masonry/proc/get_design_cost(datum/design/D)
	var/coeff = (ispath(D.build_path, /obj/item/stack) ? 1 : prod_coeff)
	var/dat
	for(var/i in D.materials)
...
/obj/item/proc/masonry_crafted(obj/machinery/autolathe/A)
	return
